...
void solve(){
    ...
    if(d<b){        // cant move the point downwards
        cout<<-1<<endl; return;
    }
    int cnt = d - b; // move vertically up, so that now we have the same y-coordinate
    a += cnt;       // a also moves up
    
    if(a<c){        // c is towards right of a, can't move there.
        cout<<-1<<endl; return;
    }
    cnt += (a-c);
    cout<<cnt<<endl;
